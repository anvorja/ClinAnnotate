...
import {UploadDropzoneProps} from "@/types/documents/uploadDropzonePropsType";
...
export function UploadDropzone({ onUploadAction }: UploadDropzoneProps) {
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        try {
            const documents = await uploadFiles(acceptedFiles);
            await onUploadAction(documents);

            toast({
                title: 'Archivos cargados',
                description: `Se cargaron ${documents.length} documentos exitosamente.`
            });
        } catch {
            toast({
                variant: 'destructive',
                title: 'Error al cargar archivos',
                description: error || 'Ocurrió un error al procesar los archivos.'
            });
        }
    }, [uploadFiles, onUploadAction, error, toast]);
