...
export default function DocumentsPage() {
    ...
    const handleUploadAction = async (newDocuments: DocumentPair[]) => {
        setDocuments(prev => [...prev, ...newDocuments]);
    };

    const handleViewAction = async (doc: DocumentPair) => {
        // Implementar vista del documento
        console.log('Ver documento:', doc);
    };

    const handleDeleteAction = async (doc: DocumentPair) => {
        setDocuments(prev => prev.filter(d => d.id !== doc.id));
    };

    const handleConvertAction = async (doc: DocumentPair, format: 'json' | 'bio') => {
        // Implementar conversión
        console.log(`Convertir documento ${doc.id} a formato ${format}`);
    };
            ...
            {documents.length === 0 ? (
                <Card className="p-12">
                    <div className="flex flex-col items-center justify-center text-center">
                        ...
                        <UploadDropzone onUploadAction={handleUploadAction} />
                    </div>
                </Card>
            ) : (
                <DocumentList
